...
const Footer = ({footerAPI: { titles, links, sociallinks}}) => {
    return (
        <>
            <footer className=" bg-gradient-to-b from-emerald-400 to-green-300
                pt-24 pb-7">
                {/* Mail alanı */}
                ...
                <div className=" w-7/12 lg:w-[95vw] m-auto mt-9"  >
                    ...
                    <div className=" flex items-center justify-between px-7 md:px-0 md:gap-5 md:flex-col-reverse">
                        <p className="text-sm md:text-center">Copyright<sub className="text-base font-bold">&copy;</sub> All Reserved Rights 2023
                        <span className=" font-semibold"> Baris B Demir </span></p>
                        <div className=" flex items-center gap-3">
                            {sociallinks?.map((val, i) => (
                                <img key={i} src={val.icon} alt="social/iconns"
                                className="w-5 h-5"/>
                            ))}
                        </div>
                    </div>
